...
def cleanup_invalid_fcm_token(invalid_token):
    ...
    try:
        success = execute_write_with_retry(
            "UPDATE couriers SET fcm_token = NULL WHERE fcm_token = ?",
            (invalid_token,)
        )
        if success:
...
def validate_all_fcm_tokens():
    """Tüm FCM token'larını validate eder ve geçersizleri temizler"""
    try:
...
def webhook_yemeksepeti():
    ...
    vendor_name = data.get("vendor_name") or data.get("restaurant_name") or data.get("vendor") or data.get(
        "merchant_name")
...
def create_restaurant():
    ...
    try:
        ...
        return jsonify(
            {"message": "Restoran ve restoran kullanıcısı oluşturuldu", "user": user_out, "restaurant": rest_out}), 201
